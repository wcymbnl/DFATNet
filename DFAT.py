...
class Detection_Transformers(object):
    _defaults = {
        ...
        #--------------------------------------------------------------------------#
        "model_path"        : 'logs/ep290-loss0.388-val_loss0.287.pth',
        "classes_path"      : 'model_data/voc_peach_class.txt',
        #---------------------------------------------------------------------#
        #   输入图片的大小
        #---------------------------------------------------------------------#
        "min_length"        : 800,
    ...
    def detect_image(self, image, crop = False, count = False):
        ...
        #   添加上batch_size维度
        #---------------------------------------------------------#
        image_data  = np.expand_dims(np.transpose(preprocess_input(np.array(image_data, dtype='float32')), (2, 0, 1)), 1)
        modified_image_data = image_data[:, 0, :, :]  # 选择第一个维度中的所有内容（所有通道）
        image_data = np.stack((modified_image_data,) * 3, axis=0)  # 复制三份通道
        image_data
        with torch.no_grad():
            ...
            #---------------------------------------------------------#
            ####改通道数
            # self.aa = nn.Conv2d(in_channels=1, out_channels=3, kernel_size=1, stride=1)
            # images = self.aa(images)
            ########
