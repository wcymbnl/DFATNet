...
def build_backbone(backbone, position_embedding, hidden_dim, train_backbone=True, pretrained=False):
    ...
    return model
# backbone: 代表主干的模型，如ResNet、EfficientNet等
# position_embedding: 表示位置编码模块，可以选择固定的按'sine'排布的位置向量或可学习的位置向量。
# hidden_dim: 表示位置向量的维度
# train_backbone: 是否对主干进行训练，默认为True
# pretrained: 是否使用预训练的主干模型，默认为False